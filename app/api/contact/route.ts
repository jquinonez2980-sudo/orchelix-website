...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!apiKey) {
      // Graceful degradation: log locally so no submissions are silently lost
      console.warn("[contact] RESEND_API_KEY not set — logging submission only.");
      console.log("[contact]", { name, company, email, phone, useCase, message, ts: new Date().toISOString() });
